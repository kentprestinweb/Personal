      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <span className="inline-block px-4 py-1.5 bg-electric-blue-100 text-electric-blue-700 text-sm font-medium rounded-full mb-4">
            {content.testimonials_label || 'Success Stories'}
          </span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-dark-900 mb-4">
            {content.testimonials_headline || 'What Local Business Owners Say'}
          </h2>
          <p className="text-dark-500 max-w-2xl mx-auto">
            {content.testimonials_description || "Don't just take my word for it – hear from the tradies and business owners I've helped grow."}
          </p>
        </div>
        ...
        {testimonials.length > 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="relative bg-white rounded-2xl shadow-lg p-8 border-2 border-dashed border-dark-200"
              >
                <div className="absolute -top-4 -left-4 w-10 h-10 bg-dark-200 rounded-full flex items-center justify-center">
                  <Quote size={18} className="text-dark-400" />
                </div>
                <div className="flex gap-1 mb-4">
                  {[...Array(5)].map((_, j) => (
                    <Star key={j} size={18} className="text-dark-200" />
                  ))}
                </div>
                <div className="space-y-2 mb-6">
                  <div className="h-4 bg-dark-100 rounded w-full"></div>
                  <div className="h-4 bg-dark-100 rounded w-5/6"></div>
                  <div className="h-4 bg-dark-100 rounded w-4/6"></div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-dark-200"></div>
                  <div className="space-y-2">
                    <div className="h-4 bg-dark-100 rounded w-24"></div>
                    <div className="h-3 bg-dark-100 rounded w-32"></div>
                  </div>
                </div>
              </div>
            ))}
